...
public class TrangThuNgan extends JFrame {
    ...
    private final LoaiSPLogic loaiSPLogic = new LoaiSPLogic();

    // =========================================================
    // CONSTRUCTOR
    // =========================================================
    public TrangThuNgan(String maNV) {
        this.maNhanVien = maNV;

        // Lấy tên nhân viên
        NhanVien nv = NhanVienDAO.getInstance().layNhanVienTheoMa(maNV);
        this.tenNhanVien = (nv != null) ? nv.getHoTen() : maNV;

        setTitle("Phần Mềm Quản Lý Siêu Thị - THỦ NGÂN (" + maNV + ")");
    ...
    private JPanel taoHeader() {
        ...
        // Badge vai trò
        pnl.add(taoBadgeVaiTro("THỦ NGÂN"));
    ...
    // =========================================================
    //  NÚT BÁN HÀNG — nổi bật trong menu
    // =========================================================
    private JPanel taoNutBanHang() {
        ...
        btnRounded.addMouseListener(new MouseAdapter() {
            @Override public void mouseEntered(MouseEvent e) { btnRounded.setBackground(CLR_BAN_HANG_HV); btnRounded.repaint(); }
            @Override public void mouseExited(MouseEvent e)  { btnRounded.setBackground(CLR_BAN_HANG);    btnRounded.repaint(); }
        });
        ...
        btnRounded.addActionListener(e -> {
            ...
            if (banHangUi != null)
                banHangUi.getPnlDanhSachSP().loadDuLieuSanPham("ALL");
    ...
    // =========================================================
    //  TẠO NHÓM MENU HÀNG HÓA (load từ DB)
    // =========================================================
    private JPanel taoNhomMenuHoverHangHoa(String tenMuc) {
        JPopupMenu popup = taoPopupDark();
        try {
            List<LoaiSP> dsLoai = loaiSPLogic.layDanhSachLoaiSP();
            if (dsLoai != null && !dsLoai.isEmpty()) {
                ...
                JMenuItem empty = taoMenuItemDark("(Chưa có danh mục)");
                empty.setEnabled(false);
                popup.add(empty);
            }
        } catch (Exception ex) {
            System.err.println("Lỗi load danh mục: " + ex.getMessage());
        }
        return taoSidebarItemVoiPopup(tenMuc, popup);
    }
