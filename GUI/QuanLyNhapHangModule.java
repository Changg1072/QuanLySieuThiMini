...
public class QuanLyNhapHangModule extends JPanel {

    private CardLayout cardLayout;
    private JPanel pnlContent;
    private PillMenu tabMenu;

    private TaoPhieuNhapUi tabTaoPhieu;
    private DanhSachNhapHangUi tabLichSu;
    private DanhSachNccUi tabNhaCungCap;

    public QuanLyNhapHangModule() {
     ...
     // 1. THANH ĐIỀU HƯỚNG TABS (Chuẩn Chrome)
        ...
        // 🔥 ĐÃ SỬA: Không xài HTML nữa, tách riêng Icon và Text để căn lề chuẩn 100%
        List<String> rawNames = List.of("LICH_SU", "TAO_PHIEU", "NCC");
        List<String> displayNames = List.of("Lịch sử nhập hàng", "Tạo phiếu nhập mới", "Quản lý nhà cung cấp");
        List<String> icons = List.of("📋", "➕", "🏢");
        List<Color> colors = List.of(new Color(245, 158, 11), new Color(16, 185, 129), new Color(59, 130, 246));

        tabMenu = new PillMenu(rawNames, displayNames, icons, colors, tabName -> {
            switch (tabName) {
                case "LICH_SU":
                    if (tabLichSu != null) { pnlContent.remove(tabLichSu); tabLichSu = new DanhSachNhapHangUi(); pnlContent.add(tabLichSu, "LICH_SU"); }
                    cardLayout.show(pnlContent, "LICH_SU");
                    break;
                case "TAO_PHIEU":
                    if (tabTaoPhieu != null) { pnlContent.remove(tabTaoPhieu); tabTaoPhieu = new TaoPhieuNhapUi(); pnlContent.add(tabTaoPhieu, "TAO_PHIEU"); }
                    cardLayout.show(pnlContent, "TAO_PHIEU");
                    break;
                case "NCC":
                    if (tabNhaCungCap != null) { pnlContent.remove(tabNhaCungCap); tabNhaCungCap = new DanhSachNccUi(); pnlContent.add(tabNhaCungCap, "NCC"); }
                    cardLayout.show(pnlContent, "NCC");
                    break;
            }
        });
        pnlHeader.add(tabMenu);
        // 2. KHU VỰC CHỨA CÁC TAB
        cardLayout = new CardLayout();
        pnlContent = new JPanel(cardLayout);
        pnlContent.setOpaque(false);

        tabLichSu = new DanhSachNhapHangUi();
        tabTaoPhieu = new TaoPhieuNhapUi();
        tabNhaCungCap = new DanhSachNccUi();

        pnlContent.add(tabLichSu, "LICH_SU");
        pnlContent.add(tabTaoPhieu, "TAO_PHIEU");
        pnlContent.add(tabNhaCungCap, "NCC");
    ...
    private class PillMenu extends JPanel {
        ...
        public PillMenu(List<String> rawNames, List<String> displayNames, List<String> icons, List<Color> colors, java.util.function.Consumer<String> onSelect) {
            ...
            for (int i = 0; i < rawNames.size(); i++) {
                ...
                btn.putClientProperty("active", i == 1); 
                ...
                btn.addActionListener(e -> { setActiveTab(rawName); onSelect.accept(rawName); });
 ...
 // Class phụ trợ vẽ Icon chuẩn kích thước
    private class EmojiIcon implements Icon {
        ...
        public EmojiIcon(String emoji, Color color, int size) { this.emoji = emoji; this.color = color; this.size = size; }
        ...
        @Override public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            // 🔥 ĐÃ SỬA: Thay vì "- 2" (giật lên), giờ đổi thành "+ 1" (hạ xuống) 
            // để Icon nằm cùng trên 1 đường thẳng ngang với chữ
