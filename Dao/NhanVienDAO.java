...
public class NhanVienDAO {
    ...
    public boolean themNhanVien(NhanVien nv) {
        // ĐÃ SỬA: Xóa LuongGio khỏi cột và VALUE
        String sql = "INSERT INTO NhanVien (MaNV, HoTen, SDT, ChucVu, TrangThai, NgayVaoLam, NgayNghiViec) " +
                     "VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try (
            ...
            ps.setNull(7, Types.DATE); // Thêm mới thì chưa có ngày nghỉ
    ...
    public boolean suaNhanVien(NhanVien nv) {
        // ĐÃ SỬA: Xóa LuongGio khỏi SET
        String sql = "UPDATE NhanVien SET HoTen=?, SDT=?, ChucVu=?, TrangThai=?, NgayVaoLam=?, NgayNghiViec=? WHERE MaNV=?";

        try (
            Connection con = ConnectDB.getInstance().getConnection();
            PreparedStatement ps = con.prepareStatement(sql)
        ) {
            // ĐIỀN ĐÚNG THỨ TỰ CỦA CÂU LỆNH UPDATE MỚI
            ps.setString(1, nv.getHoTen());
            ps.setString(2, nv.getSDT());
            ps.setString(3, nv.getChucVu());
            ps.setString(4, nv.getTrangThai());

            ps.setDate(5, (nv.getNgayVaoLam() != null) ? Date.valueOf(nv.getNgayVaoLam()) : Date.valueOf(LocalDate.now()));
            ...
            ps.setString(7, nv.getMaNV()); // WHERE MaNV = ? nằm ở vị trí số 7
